...
func enter() -> void:
	...
	turn_step_ui.ds_glow_effect.visible = true
	...
	if turn_step_ui.is_local_player:
		...
		turn_step_ui.button_exit_current_step.text = "Go to Creature Damage Step"
		
		# Tell the player handler do all the draw step functions
		Events.current_turn_step_changed.emit(self)
	

func exit() -> void:
	turn_step_ui.ds_glow_effect.visible = false
