# Creature Damage Step State
extends TurnStepState

func enter() -> void:
	# Change UI to update to current state
	turn_step_ui.cds_glow_effect.visible = true
	...
	if turn_step_ui.is_local_player:
		...
		turn_step_ui.button_exit_current_step.text = "Go to Action Step"
		
		# Tell the player handler do all the creature damage step functions
		Events.current_turn_step_changed.emit(self)
	


func exit() -> void:
	turn_step_ui.cds_glow_effect.visible = false
